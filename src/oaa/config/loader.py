...
def _load_strategy_params(cfg: dict[str, Any], root: Path) -> dict[str, Any]:
    ...
    for ref in cfg.get("strategies") or []:
        ...
        # A strategy may point at a second file for its universe (the overnight
        # pair list is generated by an offline screen, so it lives on its own).
        universe_file = ref["params"].get("pairs_file")
        if universe_file:
            universe_path = root / universe_file
            if not universe_path.exists():
                raise FileNotFoundError(
                    f"strategy '{ref.get('name')}' references a missing pairs file: "
                    f"{universe_path}. Generate it with `python scripts/find_pairs.py --write`."
                )
            loaded = _read_yaml(universe_path)
            ref["params"].setdefault("pairs", loaded.get("pairs", []))
            ref["params"]["pairs_meta"] = {
                k: v for k, v in loaded.items() if k != "pairs"
            }
    return cfg
